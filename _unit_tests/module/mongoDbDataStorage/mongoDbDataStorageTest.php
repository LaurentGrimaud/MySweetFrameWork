...
class mongoDbDataStorageTest extends PHPUnit_Framework_TestCase {
    ...
    public function setUp() {
        ...
        $this->_x->get_ready();
        $this->_x->set_iterator(new module\mongodb_iterator); //XXX used to get the test working, need to test if there is a better way to do it
        $m = new \MongoClient();
    ...
    public function test_connection_failure(){
        ...
        $configurator->expects($this->at(0))
                 ->method('inform')
                 ->with('mongo:host')
                 ->will($this->returnValue('non_existent_db'));
    ...
    public function test_successfull_retrieve($document){
        ...
        $results = iterator_to_array($this->_x->retrieve('users', $crit)); //XXX
