<?php
 ...
 //$r->set_view($v);
 //$r->reveal("home",mysfw_response::http_status_code_not_found);
 //}}}
...
//Server Error 5xx
  ...
  protected $_http_status_code= self::http_status_code_not_implemented;

  public function set_http_status_code($_) {$this->_http_status_code= $_;}
  public function get_http_status_code() {return $this->_http_status_code;}
  ...
  public function set_view(mysfw_view_interface $_) {$this->_view= $_;}
  public function get_view() {return $this->_view;}

  public function reveal($_t,$_http_status_code=200) {
   $this->set_http_status_code($_http_status_code);
   $this->_send_headers();
   $this->_view->reveal($_t);
  }
  
  private function _send_headers(){
   // headers has already been sent, do nothing but log information about headers sender
   if(headers_sent($file, $line)){
    // the following headers has already been sent by ${file}, l.${line}: print_r(headers_list(),true);
    return;
   }
   if($this->_http_status_code and self::$_http_reason_phrase[$this->_http_status_code])
    // output status line
    $status_line= "{$this->_http_version} {$this->_http_status_code} " . self::$_http_reason_phrase[$this->_http_status_code];

    header($status_line, true, $this->_http_status_code);
